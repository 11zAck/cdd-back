...
public class ConfigProd {
    ...
    // CERTIFICADOS  tbk.pem
    public static final String WEBPAY_CERT = "-----BEGIN CERTIFICATE-----\r\n" +
    ...
    public static final String COMMERCE_CODE = "597020000540";

    @Bean
    public cl.transbank.webpay.configuration.Configuration configTransbankEnvironment(){
        cl.transbank.webpay.configuration.Configuration config = new cl.transbank.webpay.configuration.Configuration();
        config.setEnvironment(Webpay.Environment.PRODUCCION );
        config.setWebpayCert( WEBPAY_CERT );
        config.setPublicCert( PUBLIC_CERT );
        config.setPrivateKey( PRIVATE_KEY );
        config.setCommerceCode( COMMERCE_CODE );
        return config;
    }
